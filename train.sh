...
do
    ...
    hash_value=$(sha256sum outputs/train.json | awk '{print $1}')

    # echo -e "STEP4:正在生成训练cache"
    ...
    if [ "$num" -eq 0 ]; then   
        CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate  launch  --main_process_port 15433 --config_file "./accelerate_config_zero3.yaml" --num_processes 8  train.py \
            ...
            --num_single_layers=0 \
            --seed=42 \
            --controlnet_model_name_or_path="train_result/flux_58k_controlnet" \
            --hash="$hash_value"
    else
    ...
    #     CUDA_VISIBLE_DEVICES=0 python3 train_controlnet_flux.py
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate  launch  --main_process_port 15433 --config_file "./accelerate_config_zero3.yaml" --num_processes 8  train.py \
            ...
            --resolution=1024 \
            --learning_rate=1e-5 \
            --max_train_steps=$(expr "$((num+1))" \* 100) \
            --checkpointing_steps=100 \
            --train_batch_size=1 \
            --gradient_accumulation_steps=8 \
            --report_to="wandb" \
    ...
    rm -r ./huggingface_home/datasets
